...
shape_parameter(k_Mpc::Real, p::EH98)::Real = shape_parameter(k_Mpc, 2.7 * p.Θ27, p.Ω_m0_h2)

function transfer_cdm(k_Mpc::Real, p::EH98)::Real
...
function transfer_function(k_Mpc::Real, p::EH98 = EH98_planck18; with_baryons::Bool = true)
    tc = transfer_cdm(k_Mpc, p)
    tb = transfer_baryons(k_Mpc, p)
    return with_baryons ? p.Ω_b0_h2 / p.Ω_m0_h2 * tb + p.Ω_χ0_h2/p.Ω_m0_h2 * tc : p.Ω_χ0_h2 / p.Ω_m0_h2 * tc
end
